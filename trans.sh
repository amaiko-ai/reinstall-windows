...
modify_windows() {
    ...
    # 4.5 OpenSSH —— amaiko fork addition, not present upstream.
    #
    # AFTER the netconf step above and before everything else, because sshd is
    # useless until the NIC is configured, and because every later step is
    # easier to debug once there is a shell on the box.
    #
    # Unconditional: this fork exists to build a headless CI runner, and a
    # runner nobody can reach is not a runner. Upstream gates comparable steps
    # behind flags; there is no flag here on purpose, so it cannot be forgotten.
    download $confhome/windows-openssh.bat $os_dir/windows-openssh.bat
    bats="$bats windows-openssh.bat"
