...
set -eE
# AMAIKO FORK. Repointed at this fork, because the whole reason the fork exists
# is that this value is hardcoded: there is no flag and no environment variable
# to override it (grep the tree for CONFHOME — nothing). Every helper file the
# installer fetches at run time comes from here, including windows-openssh.bat,
# which upstream does not have.
#
# Upstream's commit-pinning block further down is dead code — it is guarded by
# `if false &&` and marked 未测试 — so this value is used verbatim and whatever
# ref it names is what the install actually consumes. It is therefore pinned to
# a commit rather than a branch, and pinned by the commit that follows this one:
...
# helper files from a mirror of UPSTREAM, which has no windows-openssh.bat, so
# the download fails, the install completes anyway, and the box comes up with no
# SSH and no NetBird — reachable only by RDP. Empty makes the code fall through
# to the github_proxy branch, which rewrites the host but keeps this fork's path.
confhome_cn=
